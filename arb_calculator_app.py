...
def get_orderbook(token_id):
    try:
        ...
        # Manche Polymarket-Märkte haben kein CLOB-Orderbook
...
def find_equal_payout(selected, bankroll):
    """
    Kauft virtuell gleich viele YES-Shares pro Team.
    Wenn eines der Top-N Teams gewinnt, ist der Payout gleich.
    """
...
if st.button("Berechnen"):
    ...
    try:
        ...
        if len(teams) < int(top_n):
            st.error("Nicht genug handelbare Orderbook-Märkte für diese Top-N Auswahl gefunden.")
            st.stop()

        # Top Teams = höchste YES-Wahrscheinlichkeit = höchster Best Ask
        teams.sort(key=lambda x: x["best_ask"], reverse=True)

        selected = teams[:int(top_n)]

        payout, real_cost, stake_rows = find_equal_payout(selected, float(bankroll))
        ...
        st.metric("Effektive Summe", f"{effective_sum:.4f}")
